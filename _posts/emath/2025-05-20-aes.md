...
AES follows a structure known as Substitution Permutation Network (SPN) . A general illustration will be provided here <>. 
...
Using a substitution box (S-box), each element in the state matrix is non-linearly transformed (see image).
2.	Permutation: Permutation in AES is divided into two steps:
...
AddroundKey  9 rounds of the above  a final round of the above without Mixcolumns.
